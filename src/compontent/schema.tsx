      ...
      <div className="relative p-5 my-2 bg-white rounded-md shadow-md">
        <div className="flex items-center justify-between gap-4 my-3 text-sm font-semibold">
          <ul className="flex gap-2">
            <li>
              <button onClick={()=>{setSchema(1)}} className={`border-b-2 ${schema === 1?"text-[#7c74ff] border-[#7c74ff]":"text-[#b3b0c6] border-transparent"}`}>Schema</button>
            </li>
            <li>
              <button  onClick={()=>{setSchema(2)}} className={`border-b-2 ${schema === 2 ?"text-[#7c74ff] border-[#7c74ff]":"text-[#b3b0c6] border-transparent"}`}>Hreflang</button>
            </li>
          </ul>
          <button className="flex items-center gap-2 bg-zinc-300 rounded-md px-4 py-1.5">
            <svg
              width="15"
              height="15"
              viewBox="0 0 15 15"
            ...
            >
              <g clip-path="url(#clip0_3501_5018)">
                <path
        ...
        </div>
        {
          ...
          <p>Schema is a form of microdata which helps add context for search engines regarding what a web page is about. You don't need to have it, though it has many use cases. It's usually fine if this tab is empty.</p>
        </div>:<></>
        }
        {
          schema === 2?
          <div className="text-sm text-zinc-400">
          <p>Hreflang is a markup attribute used in website coding to indicate the language and regional targeting of a web page. It helps search engines understand which language versions of a page should be shown to users in different regions. While not mandatory, implementing Hreflang can improve the accuracy of search engine results for multilingual websites and enhance the user experience for international audiences.</p>
        </div>:<></>
        }        
      </div>
    </>
