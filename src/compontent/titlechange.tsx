...
export default function TitleChange({ Back }: TitleChangeProps): JSX.Element {
  const [text, setText] = useState<string>("");

  const wordCount: number = text.trim() === "" ? 0 : text.trim().split(/\s+/).length;
  const charCount: number = text.length;

  const isMetaTitleInvalid: boolean = charCount < 1 || charCount > 65;
  ...
  return (
    <div className="relative p-5 mb-2 bg-white dark:bg-[#1d1d1d] rounded-md shadow-md">
      <button className="flex items-center w-full gap-1 mb-2">
        ...
        <span onClick={Back}>Edit Title</span>
        <svg
